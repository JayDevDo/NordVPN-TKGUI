...
def meshPort( cmdArgs:[] ):
	...
	try:
		if len(cmdArgs) > 1:
			...
			# print(f"meshPort: --meshPortResp(RAW): { str( meshPortResp ) }")
		else:
			print(f"meshPort| cmdArgs must have at least 2 items ('nordvpn', 'meshnet')")
			meshPortResp = ["cmdArgs must have at least 2 items"]

	except Exception as meshPortExcptn:
		print(f"meshPort Exception = { meshPortExcptn }")
		meshPortResp = [f"meshPortResp { cmdArgs} got Exception { meshPortExcptn }"]
