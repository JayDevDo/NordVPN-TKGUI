...
class TabSettings( tk.Frame ):
	...
	def __init__( 
		...
		self.tabSttngs_GridFrm = tk.LabelFrame(
			...
			bg = skin.myBlack,
			width 	= self.dimensions[0], # 100,
			height 	= self.dimensions[1], # 100, 
			labelanchor = 's'
		)
	...
	def sttngChangeClicked( self , event, args ):
		...
		# if setting is meshnet, refresh meshnet tab via pubsub
		self.getLiveSettings()
	...
	def doLayOut( self ):
		...
		for i,s in enumerate(self.tabSttngs_Data):
			...
			if sttngType == type(True) :
				...
				print(f"setting type it's something else, stick it in a label { sttngType }")
