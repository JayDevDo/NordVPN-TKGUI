...
class ListBoxDialog:
	...
	def __init__( self, parent ):
		...
		# geometry ("width + height + xpos + ypos") as string
		dlgWinDimensiom = [350,250] # [width,height]
		dlgParentX = 600
		dlgParentY = 350

		self.geoString = f"{ dlgWinDimensiom[0] }x{ dlgWinDimensiom[1] }+{ dlgParentX }+{ dlgParentY }"
		self.top.geometry( self.geoString )
		self.top.configure( background = skin.myBlack )
	...
	def lbUpdate( self, event ):
		...
		if ( len( self.ratingLB.curselection() ) > 0 ):
			...
			print(f"ListBoxDialog | lbUpdate --self.geoString: { self.geoString } --pickedItem: { pickedItem }")
